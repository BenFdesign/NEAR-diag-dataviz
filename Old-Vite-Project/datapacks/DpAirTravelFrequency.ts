...
// Interface for a single air travel frequency choice response
...
// Interface for air travel frequency data for a single SU
...
// Constants
const DATAPACK_NAME = 'DpAirTravelFrequency';
const QUESTION_KEY = 'Air Travel Frequency';

// Global cache
let precomputedCache: PrecomputedAirTravelFrequencyData | null = null;

// Get air travel frequency choices from metadata using Metabase Question Key
...
// Calculate air travel frequency distribution for a specific SU  
...
// Pre-compute all air travel frequency data following the established rules
...
export function runAirTravelFrequencyTests(): boolean {
  ...
  try {
    ...
    const data2 = fetchAirTravelFrequencyData();
    ...
    const quartierData = fetchAirTravelFrequencyData([]);
    const suData = fetchAirTravelFrequencyData([1]);
...
// Run tests manually by calling runAirTravelFrequencyTests() when needed