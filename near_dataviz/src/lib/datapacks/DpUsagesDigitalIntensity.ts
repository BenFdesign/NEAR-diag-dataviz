...
const getDigitalIntensityChoices = (metaSuChoicesData: MetaChoice[]): MetaChoice[] => {
...
const calculateDigitalIntensityForSu = (
  suAnswerData: SuAnswerRecord[],
  metaSuQuestionsData: MetaQuestion[],
  metaSuChoicesData: MetaChoice[],
  suLocalId: number
): DigitalIntensityData => {
  const choices = getDigitalIntensityChoices(metaSuChoicesData)
...
const precomputeAllDigitalIntensityData = (
  suAnswerData: SuAnswerRecord[],
  metaSuQuestionsData: MetaQuestion[],
  metaSuChoicesData: MetaChoice[],
  suData: SuRecord[]
): PrecomputedDigitalIntensityData => {
  ...
  allSuLocalIds.forEach((suLocalId: number) => {
    const suResult = calculateDigitalIntensityForSu(
      suAnswerData,
      metaSuQuestionsData,
      metaSuChoicesData,
      suLocalId
...
const getPrecomputedData = async (): Promise<PrecomputedDigitalIntensityData> => {
...
export async function fetchDigitalIntensityData(selectedSus?: number[]): Promise<DigitalIntensityResult> {
...
export async function runDigitalIntensityTests(): Promise<boolean> {
...
export async function getDpUsagesDigitalIntensityDatapack(
  request: DatapackRequest
): Promise<DatapackResponse<DigitalIntensityResult>> {
  const selectedSus = request.selectedSus

  const result = await fetchDigitalIntensityData(selectedSus)
  ...
  const response: DatapackResponse<DigitalIntensityResult> = {
    id: DATAPACK_NAME,
    version: '1.0.0',
    data: result,
    context,
